...
- [ ] Configure local Android upload signing, its expected certificate hash, and Play service-account access as described in `docs/local-release.md`.
- [ ] Back up the Android keystore securely and retain the local build records and exact artifacts for recovery.
- [ ] Before each local Play upload/promotion, confirm Publishing overview has no unrelated changes and pause other Console/Publisher writers until completion.
...
Native app version / phone and Wear code pair / local build record:
