...
The `Publish Expo OTA Update` workflow runs whenever a commit lands on `master`. It publishes Android phone
JavaScript/assets to the `internal` channel first, then starts a dependent production job. The production job targets
the protected GitHub `production` environment, so it remains blocked until a required reviewer verifies the internal
devices and approves the deployment. A newer `master` commit cancels an older pending promotion so stale code cannot
be approved after a newer internal update.
...
The workflow pulls and validates each selected EAS environment, then publishes the merge commit with its commit
message. No native build tag is supplied. Expo selects clients by project, Android platform, channel, and the
`appVersion` runtime generated by `mobile/app.config.js`; clients on an older or otherwise different runtime do not
receive the update. Increment the mobile app version and create a new signed native build whenever native
dependencies, config plugins, icons, permissions, or other native inputs change. OTA updates never update the Wear
app.

After testing the automatic internal deployment, open its workflow run in GitHub Actions and approve the waiting
`production` environment deployment. Reject it or leave it pending if verification does not pass.
