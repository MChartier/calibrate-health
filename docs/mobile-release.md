...
its bundled server release line with uncached `/api/v1/client-config` before restoring a session or synchronizing. A
...
Both jobs check out the exact commit selected at dispatch, even if `master` advances while production approval is
pending. Each job verifies that commit descends from `native_build_ref`, pulls and validates its selected EAS
environment, and compares the update source with the installed native build. The `appVersion` and native fingerprint
...
After testing the dispatched internal deployment, open its workflow run in GitHub Actions and approve the waiting
