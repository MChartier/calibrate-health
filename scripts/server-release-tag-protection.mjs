...
export const SERVER_RELEASE_TAG_REF_PATTERN = 'refs/tags/v*';
export const SERVER_RELEASE_TAG_RULESETS = Object.freeze({
  creation: Object.freeze({
    name: 'server-release-tag-creation',
    ruleTypes: Object.freeze(['creation'])
  }),
  immutability: Object.freeze({
    name: 'server-release-tag-immutability',
...
const SERVER_RELEASE_TAG_PATTERN = /^v\d+\.\d+\.\d+$/;
const REPOSITORY_PATTERN = /^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/;

function requireServerReleaseTag(tag) {
  if (typeof tag !== 'string' || !SERVER_RELEASE_TAG_PATTERN.test(tag)) {
    throw new Error('Server release tag protection requires --tag vMAJOR.MINOR.PATCH.');
  }
  return tag;
}
...
function parseRepository(repository) {
  if (typeof repository !== 'string' || !REPOSITORY_PATTERN.test(repository)) {
    throw new Error('Server release tag protection requires --repository OWNER/REPO.');
  }
...
function requireExactRefCondition(ruleset) {
  ...
  if (
    ...
    refName.include[0] !== SERVER_RELEASE_TAG_REF_PATTERN
  ) {
    throw new Error(`${ruleset.name} must include exactly ${SERVER_RELEASE_TAG_REF_PATTERN}.`);
  }
  if (!Array.isArray(refName.exclude) || refName.exclude.length !== 0) {
    throw new Error(`${ruleset.name} must not exclude any stable server release tags.`);
  }
}
...
/**
 * Validate the ruleset shape visible through GitHub's read API. Bypass actors
 * are not visible to this token and must be verified separately during setup.
 */
export function validateServerReleaseTagProtection(rulesets, options = {}) {
  const tag = requireServerReleaseTag(options.tag);
  ...
  const verified = {};
  for (const [key, requirement] of Object.entries(SERVER_RELEASE_TAG_RULESETS)) {
  ...
  return Object.freeze({
    tag,
    refPattern: SERVER_RELEASE_TAG_REF_PATTERN,
...
export async function fetchServerReleaseTagRulesets(options = {}) {
  const { owner, repo } = parseRepository(options.repository);
  const token = typeof options.token === 'string' ? options.token.trim() : '';
  if (!token) throw new Error('GITHUB_TOKEN is required to verify server release tag protection.');
  ...
  for (const { name } of Object.values(SERVER_RELEASE_TAG_RULESETS)) {
...
export async function verifyServerReleaseTagProtection(options = {}) {
  const tag = requireServerReleaseTag(options.tag);
  const rulesets = await fetchServerReleaseTagRulesets(options);
  return validateServerReleaseTagProtection(rulesets, { tag });
}
...
export async function runServerReleaseTagProtectionCli(options = {}) {
  ...
  const output = `Visible server release tag ruleset shape verified for ${repository} ${tag}.\n`;
  (options.stdout ?? process.stdout).write(output);
  return result;
}

if (process.argv[1] && pathToFileURL(path.resolve(process.argv[1])).href === import.meta.url) {
  runServerReleaseTagProtectionCli().catch((error) => {
    console.error(`[server-release-tag-protection] ${error instanceof Error ? error.message : error}`);
    process.exitCode = 1;
  });
}
